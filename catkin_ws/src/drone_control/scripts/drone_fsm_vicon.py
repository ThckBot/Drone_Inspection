...
class DroneFSM():
    ...
    def vicon_callback(self, pose_msg):
        # has x,y,z
        self.trans = pose_msg.transform.translation
        # quaternion
        self.rot = pose_msg.transform.rotation
    ...
    def takeoff(self, height):
        print("Takeoff...")
        self.sp = self.trans
        while self.trans.z < height:
            self.sp.z = min(self.trans.z + 0.02,height)
    ...
    def land(self):
        print("Landing...")
        self.sp = self.trans
        while self.trans.z > 0.05:
            self.sp.z = self.trans.z-0.05
            self.publish_setpoint(self.sp)
            self.rate.sleep()
        # self.stop()
